...
function buildStaticReportHtml(payload, opts) {
  ...
  // ★★ 这三处**必须用函数替换器** `() => 片段`，不能直接传字符串。
  //   `String.prototype.replace` 会把替换串里的 `$&`、`` $` ``、`$'` 当成特殊模式展开，
  //   而 data 里含远端可控字符串（游戏名、组员昵称）以及离线链路下的任意字段：
  //   组里一个人把昵称改成 `bob$` + 反引号，整份报告就会被撑大、数据块变成语法错误、
  //   页面全白 —— 而且生成期**不报错**（含 `$&` 时抛的还是「脚本还是外链」这种指错地方的错）。
  //   改成函数之后，返回值里的 `$` 一律按字面量处理。
  const out = html
