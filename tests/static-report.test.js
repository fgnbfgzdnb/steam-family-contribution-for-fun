...
(async function main() {
  ...
  test('★ warnings 不进产物（远端可控文本不能随报告转发出去）', () => {
    ...
    /* ★★ 为什么另起一组：`String.prototype.replace` 的**字符串**替换器会把替换串里的
     *   `$&`、`$` + 反引号、`$'` 当成特殊模式展开，而报告正是把 JSON 拼进替换串的。
     *   组里一个人把昵称改成这些序列，整份报告就会被撑大、内联数据块变成语法错误、页面全白 ——
     *   而且生成期**不报错**（含 `$&` 时抛的还是「脚本还是外链」这种指错地方的消息）。
     *   这里锁三件事：产物别膨胀、脚本标签开闭配平、数据块能被解析。 */
    const cleanPayload = JSON.parse(JSON.stringify(fixtureRaw.family ? fixtureRaw.family : fixtureRaw));
    const baseline = buildStaticReportHtml(cleanPayload, { generatedAt: '' });

    [
      ['成员昵称含 $ + 反引号', function (p) { p.members[0].name = 'bob$' + String.fromCharCode(96); }],
      ['成员昵称含 $ + 单引号', function (p) { p.members[0].name = "C$'D"; }],
      ['成员昵称含 $&', function (p) { p.members[0].name = 'C$&D'; }],
      ['游戏名含 $ + 反引号', function (p) { p.games[0].name = 'X$' + String.fromCharCode(96); }]
    ].forEach(function (row) {
      test('★ ' + row[0] + ' 不能让报告结构坏掉', function () {
        const p = JSON.parse(JSON.stringify(cleanPayload));
        row[1](p);
        const html = buildStaticReportHtml(p, { generatedAt: '' });
        assert.ok(html.length <= baseline.length * 1.2,
          '产物异常膨胀：' + html.length + ' 字节，基线 ' + baseline.length);
        assert.strictEqual((html.match(/<script/g) || []).length, (html.match(/<\/script/g) || []).length,
          '<script> 与 </script> 数量必须相等（不等 = 替换串里的 $ 序列被展开了）');
        const m = html.match(/window\.__FAMILY_DATA__ = ([\s\S]*?);<\/script>/);
        assert.ok(m, '抽不到内联数据块');
        new Function('return ' + m[1]);   // 语法错误会直接抛出来
      });
    });

    test('对照组：干净夹具的数据块本来就能解析（证明上面那组不是哑的）', function () {
      const m = baseline.match(/window\.__FAMILY_DATA__ = ([\s\S]*?);<\/script>/);
      assert.ok(m, '干净产物里也应能抽到内联数据块');
      new Function('return ' + m[1]);
      assert.strictEqual((baseline.match(/<script/g) || []).length, 3, '干净产物应有 3 个 <script>');
    });

    /* ★ 顶层 `warnings` 是离线 `--from` 链路的主防线，而它原先只有**嵌套**那一层有用例。
     *   `--from` 吃的那份 JSON 可以是别人递过来的，什么字段都可能有 —— 这里两种形状都锁一遍。 */
    test('★ warnings 两道都要剔：顶层与嵌套 priceMeta（离线 --from 的主防线）', function () {
      const top = JSON.parse(JSON.stringify(cleanPayload));
      top.warnings = ['回显：https://api.steampowered.com/x?access_token=TOP-LEAKED'];
      const h1 = buildStaticReportHtml(top, { generatedAt: '' });
      assert.ok(h1.indexOf('TOP-LEAKED') < 0, '顶层 warnings 被内联进产物了');

      const nested = JSON.parse(JSON.stringify(cleanPayload));
      nested.priceMeta = {
        source: 'store',
        degraded: true,
        rates: { HKD: 0.91 },
        warnings: ['回显：https://api.steampowered.com/y?access_token=NEST-LEAKED']
      };
      const h2 = buildStaticReportHtml(nested, { generatedAt: '' });
      assert.ok(h2.indexOf('NEST-LEAKED') < 0, '嵌套 warnings 被内联进产物了');
      assert.ok(h2.indexOf('"degraded":true') >= 0 && h2.indexOf('"HKD":0.91') >= 0,
        '除 warnings 外的元信息要保留 —— 剔的是那个字段，不是整块元信息');
    });
