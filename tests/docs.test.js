...
test('★ README 里的 STEAM_* 配置项名只能来自白名单', function () {  // 只看 STEAM_ 前缀：这一族的名字才是「本项目的配置项」，
...
/* ★ 知情同意那一段的锚点。
 *   README 的「与 Steam 服务条款的关系」是全仓**唯一**的风险披露，而它现在没有任何测试钉着 ——
 *   按本仓库惯例必须补锚点，否则后人整理文档时最容易被当「废话」删掉。
 *   ★ 只钉「必须说」是不够的：还要钉「不许说」—— 把一个灰区说成没风险，属于不实陈述。
 *   needle 一律拆开拼（照 FORBIDDEN_FORMS 的写法），否则本文件自己会被自己的扫描扫红。 */
console.log('\n[文档] 知情同意：风险披露该说的必须说到、不该说的不许出现');
...
[['条款编号', '4' + '.C'],
  ...
  test('★ README 的风险披露必须包含「' + row[0] + '」', function () {
    assert.ok(README.indexOf(row[1]) >= 0,
      'README 里少了「' + row[0] + '」—— 这一段是全仓唯一的风险披露，别把它压缩成空话');
  });
});

['不会有任何风险', '绝对安全', '官方' + '允许', 'Valve' + '认可'].forEach(function (bad) {
  test('★ README 不许出现反向保证：' + bad, function () {
    assert.ok(README.indexOf(bad) < 0,
      'README 里出现了反向保证 `' + bad + '` —— 这是把一个灰区说成没风险，属于不实陈述');
  });
});
